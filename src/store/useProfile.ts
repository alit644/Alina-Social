  ...
  updateProfile: async (data: any) => {
    ...
    try {
     ...
     if (data.avatar instanceof File) {
      ...
      // جلب الرابط العام
      const { data: publicUrlData } = supabase.storage.from('profile-image').getPublicUrl(fileName);
      avataUrl = publicUrlData.publicUrl;
     }
      
     const {error:updateErr, data:updatedProfile} = await supabase.from("profiles").update({
